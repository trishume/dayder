...
pub fn pearson_correlation_coefficient(xs: &Vec<f32>, ys: &Vec<f32>) -> f64 {
    ...
    let denom = (sum_xx - (n as f64) * (x_avg * x_avg)).sqrt()
        * (sum_yy - (n as f64) * (y_avg * y_avg)).sqrt();
...
pub fn pairinate(
    base: &BinaryTimeSeries,
    other: &BinaryTimeSeries,
) -> Option<(Vec<f32>, Vec<f32>)> {
    if base.data.len() < MIN_OVERLAP || other.data.len() < MIN_OVERLAP {
        return None;
    }
    let (bs, os, sampled_other) = interpolate(&base.data[..], &other.data[..]);
    if bs.len() < MIN_OVERLAP {
        return None;
    }
    if sampled_other < MIN_OVERLAP {
        return None;
    }
    return Some((bs, os));
}
...
fn interpolate(base: &[Point], other: &[Point]) -> (Vec<f32>, Vec<f32>, usize) {
    ...
    for data_point in base {
        ...
        while other[search_index].t < data_point.t
            && search_index < (other.len() - 1)
            && other[search_index + 1].t <= data_point.t
        {
            search_index += 1;
            advanced_other = true;
        }
