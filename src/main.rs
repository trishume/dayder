...
use iron::{Iron, IronError, IronResult, Plugin, Request, Response};
...
impl Key for CorrCache {
    type Value = CorrelationCache;
}

lazy_static! {
    static ref DATA_SETS: Vec<lib::btsf::BinaryTimeSeries> = {
        let mut all_data = Vec::new();
        lib::btsf::read_btsf_file(
            &mut File::open("./btsf/mortality.btsf").unwrap(),
            &mut all_data,
        )
        .unwrap();
        lib::btsf::read_btsf_file(
            &mut File::open("./btsf/canada_gdp.btsf").unwrap(),
            &mut all_data,
        )
        .unwrap();
        if Path::new("./btsf/fred-small.btsf").exists() {
            lib::btsf::read_btsf_file(
                &mut BufReader::new(File::open("./btsf/fred-small.btsf").unwrap()),
                &mut all_data,
            )
            .unwrap();
        }
    ...
    fn find_handler(req: &mut Request) -> IronResult<Response> {
        ...
        if input_charts.len() != 1 {
            return Ok(Response::with((
                status::BadRequest,
                "Please send a BTSF file with precisely one chart in it",
            )));
        }
        ...
        let filtered: Vec<lib::btsf::CorrelatedTimeSeries> = if filter != "" {
            ...
            SET_NAMES
                ...
                .map(|(i, _)| lib::btsf::CorrelatedTimeSeries {
                    series: &DATA_SETS[i],
                    correlation: result[i],
                })
            ...
            result
                .iter()
                .enumerate()
                .map(|(i, c)| lib::btsf::CorrelatedTimeSeries {
                    series: &DATA_SETS[i],
                    correlation: c.clone(),
                })
                .filter(|corr_series| corr_series.correlation != 0.0)
        ...
        p.end_frame();
        println!(
            "Find handler, filter: '{}', corr: '{}'",
            filter, input_charts[0].name
        );
        p.print_timing();
